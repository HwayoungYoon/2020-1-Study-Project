...
1. 배열을 만든다는게 아래의 코드처럼 질문과 배열을 따로 만들어야 하나요? 아니면 질문리스트의 형태 자체를 배열로 만들어서 사용해야 하나요?
2. calcMax에서 if문과 for문 안에 들어갈 조건을 어떻게 써야할지 모르겠습니다....
3. 아래 코드가 맞는(제대로 작동하는) 코드인가요...?
 */
const select = [];
const qnaArray = [
...
/* 
1. 질문 및 답변 형식 2인 경우에도 기존처럼 선택한 답변에 해당하는 결과에 점수를 부여하고, 
마지막까지 선택이 끝나면 각각 점수가 부여된 12개의 결과 중 최대값을 가지는 결과를 출력할 수 있나요?
2. 질문 및 답변 형식 2인 경우에 각 답변에 해당하는 id를 배열의 형태로 만들어서 
선택된 답변의 배열들을 concat으로 합친 배열을 만들고 
새로 만들어진 배열에서 원소들의 빈도가 가장 높은 값을 찾아서 반환하고 
반환된 값이 point_5이면 point에 500점을, point_11이면 point에 1100점을 부과할 수 있도록 만들 수도 있나요?

아래 코드는 맞는지 확신이 없지만 선택된 답변들의 id들을 합친 새로운 배열 pointArray를 만드는 코드입니다.
*/
const calcMax = () => {
    let pointArray = [];
